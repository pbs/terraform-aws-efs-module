...
```hcl
module "efs" {
    source = "github.com/pbs/terraform-efs-module?ref=x.y.z"
}
```
...
```bash
mkdir -p 'terraform/modules/efs'
gh -R 'pbs/terraform-efs-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/efs'
rm -f release.tar.gz
```
...
```bash
git remote add -f efs git@github.com:pbs/terraform-efs-module.git
git subtree add --prefix terraform/modules/efs efs main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/efs efs $REF --squash
```
